...
namespace ZedGraph
{
	...
	#region Rendering Methods
        ...
        public Bitmap GetImage(int width, int height, bool isAntiAlias, int rightMargin, int bottomMargin)
        {
            Bitmap bitmap = new Bitmap(width, height);
            using (Graphics bitmapGraphics = Graphics.FromImage(bitmap))
            {
                //SetAntiAliasMode(bitmapGraphics, isAntiAlias);
                GraphPane pane = (GraphPane)this.ShallowClone();
                pane.ReSize(bitmapGraphics, new RectangleF(0, 0, width, height));
                RectangleF rec = new RectangleF(new PointF(pane._rect.X + 60, pane._rect.Y + 20), new SizeF(width - rightMargin, height - bottomMargin));
                pane.Chart.Rect =  rec;
                pane.Draw(bitmapGraphics);
            }
