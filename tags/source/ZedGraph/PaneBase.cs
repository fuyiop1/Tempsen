...
namespace ZedGraph
{
	...
	#region Methods
        ...
        public virtual Bitmap GetImage(int width, int height, bool isAntiAlias)
        {
            ...
            using (Graphics bitmapGraphics = Graphics.FromImage(bitmap))
            {
                ...
                if (tempPane._rect != RectangleF.Empty)
            {
                RectangleF rec = new RectangleF(new PointF(tempPane._rect.X, tempPane._rect.Y + 20), new SizeF(width - 110, height - 100));
                //tempPane.Chart.Rect = rec;
            }
...
/*
		...
		*/
		internal PointF TransformCoord( double x, double y, CoordType coord )
		{
            ...
            else if (coord == CoordType.XChartScaleYScale)
            {
                ptPix.X = (float)(chartRect.Right + x);
                ptPix.Y = gPane.YAxis.Scale.Transform(y);
            }
