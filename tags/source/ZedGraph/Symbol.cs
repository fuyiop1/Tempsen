...
namespace ZedGraph
{
	...
	#region Fields
        ...
        private bool _isDrawSingleSymbol = false;


        private int _DrawSymbolIndex = 1;
        private float m_SybmbolBaseValue = 100F;
	...
	#region Properties
        ...
        /// <summary>
        /// is draw a single symbol
        /// </summary>
        public bool IsDrawSingleSymbol
        {
            get { return _isDrawSingleSymbol; }
            set { _isDrawSingleSymbol = value; }
        }
        /// <summary>
        ///when IsDrawSingleSymbol is true this valid
        /// </summary>
        public int DrawSymbolIndex
        {
            get { return _DrawSymbolIndex; }
            set { _DrawSymbolIndex = value; }
        }
        public List<int> MarkedSymbolList { get; set; }
        public SymbolType MarkedSymbolType { get; set; }
        public Color MarkedSymbolColor { get; set; }
        /// <summary>
        /// to control the marksymbol size
        /// </summary>
        public float SybmbolBaseValue
        {
            get { return m_SybmbolBaseValue; }
            set { m_SybmbolBaseValue = value; }
        }
		#endregion
	...
	#region Rendering Methods
        ...
        public GraphicsPath MakePath(Graphics g, float scaleFactor, SymbolType markSymbol)
        {
            float scaledSize = (float)(_size * scaleFactor);
            float hsize = scaledSize / 2,
                    hsize1 = hsize + 1;
			...
			if ( points != null && ( _border.IsVisible || _fill.IsVisible ) )
			{
				...
				using ( GraphicsPath path = MakePath( g, scaleFactor ) )
				{
					...
					using ( Brush brush = source.Fill.MakeBrush( rect ) )
					{
                        ...
                        int end = 1,start;
                        if (_isDrawSingleSymbol)
                        {
                            end = _DrawSymbolIndex+1;
                            start = _DrawSymbolIndex;
                        }
                        else
                        {
                            end = points.Count;
        ...
        public void DrawMark(Graphics g, GraphPane pane, LineItem curve, float scaleFactor,
            bool isSelected)
        {
            if (MarkedSymbolList == null)
                return;
            //scaleFactor = pane.Chart.Rect.Width / (pane.BaseDimension * SybmbolBaseValue);
            scaleFactor = 1.83625f;
            Symbol source = this;
            if (isSelected)
                source = Selection.Symbol;
            ...
            if (points != null && (_border.IsVisible || _fill.IsVisible))
            {
                ...
                using (Pen pen = source._border.GetPen(pane, scaleFactor))
                using (GraphicsPath path = MakePath(g, scaleFactor, MarkedSymbolType == null ? SymbolType.Default : MarkedSymbolType))
                {
                    RectangleF rect = path.GetBounds();
                    Fill tempFill = new Fill(MarkedSymbolColor);
                    using (Brush brush = tempFill.MakeBrush(rect))
                    {
                        ValueHandler valueHandler = new ValueHandler(pane, false);
                        Scale xScale = curve.GetXAxis(pane).Scale;
                        Scale yScale = curve.GetYAxis(pane).Scale;
                        ...
                        if (MarkedSymbolList != null)
                        {
                            foreach (int i in MarkedSymbolList)
                            {
                                if (i >= points.Count)
                                    return;
                                // Get the user scale values for the current point
