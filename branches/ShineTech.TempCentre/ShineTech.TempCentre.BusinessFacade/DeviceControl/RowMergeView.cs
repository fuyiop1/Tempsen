...
using System.Runtime.InteropServices;
    ...
    public partial class RowMergeView : DataGridView
    {
                ...
                else
                {
                    ...
                    if (e.RowIndex == -1)
                    {
                        if (SpanRows.ContainsKey(e.ColumnIndex)) //被合并的列
                        {
                            ...
                            {
                                if (left > right)
                                    left = LeftSize(e.ColumnIndex,left,right);
        ...
        private int LeftSize(int index,int left,int right)
        {

            left = 79 * (index - 5);
            if (left > right)
                left = LeftSize(index - 3, left, right);
            return left;
        }
        ...
        public Dictionary<string, bool> MergeColIsShow
        {
            get{return _MergeColIsShow;}
            set { _MergeColIsShow = value; }
        }
        private Dictionary<string, bool> _MergeColIsShow = new Dictionary<string, bool>();
        #endregion
        #region 二维表头
