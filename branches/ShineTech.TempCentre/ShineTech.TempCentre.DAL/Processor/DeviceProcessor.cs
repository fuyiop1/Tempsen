...
namespace ShineTech.TempCentre.DAL
{
    public class DeviceProcessor : IDataProcessor
    {
                ...
                ds.Tables[0].Rows.OfType<DataRow>().ToList().ForEach(row =>
                {
                    ...
                    properityInfo.ForEach(p =>
                    {
                        if (ds.Tables[0].Columns.Contains(p.Name))
                            ...
                            p.SetValue(obj,row[p.Name]==DBNull.Value?null:row[p.Name], null);
                    });
                    list.Add(obj);
                });
                return list;
        ...
        public int  ExecuteNonQuery(string cmdText, Dictionary<string, object> o)
        {
            return ExecuteNonQuery(cmdText, null, o);
        }
        public int ExecuteNonQuery(string cmdText, DbTransaction tran, Dictionary<string, object> o)
        {
            if (o != null && o.Keys.Count > 0)
            {
                List<DbParameter> list = new List<DbParameter>();
                ...
                return SQLiteHelper.SQLiteHelper.ExecuteNonQuery(tran,cmdText, list.ToArray());
                //SQLiteHelper.SQLiteHelper.CreateParameter(
            }
            else
                return SQLiteHelper.SQLiteHelper.ExecuteNonQuery(tran, cmdText, null);
        }
